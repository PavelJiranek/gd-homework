...
const LoginFormComponent: React.FC<MyFormProps & FormikProps<FormValues>> = (props) => {
  const { values, touched, errors, isSubmitting, handleChange, handleBlur, handleSubmit, loginError } = props;
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className={cx(styles.Login, 's-login-form')}>
        ...
        <div className={styles.InputBlock}>
          <Label className={styles.Label} hasError={!!errors.password && touched.password} htmlFor="password">
            Password
          </Label>
          <Input
